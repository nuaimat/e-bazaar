...
public class FourByTwoGridPane extends GridPane {
	...
	public FourByTwoGridPane(List<String> rowHeaders, TextField[] fieldValues, 
			String borderColor, int minWidth) {
		this.rowHeaders = rowHeaders;
	...
	public FourByTwoGridPane(List<String> rowHeaders, List<? extends Node> nodeValues, 
			String borderColor, double minWidth) {
		this.rowHeaders = rowHeaders;
	...
	private void makeNodeRows(){
		for(int i = 0; i < 4; ++i)  {
			addRow(i, new Label(rowHeaders.get(i)), nodeValues.get(i));
		}
